(function() {
    ...
    var a = 1200, c = 800, h = c, U = 200, K = 22, S = 20, s = 8, R = 110, J = 20, o = 15, t = 10, w = 600, F = "elastic", N = "#0da4d3";
    var T, q, x, j, H, A, P;
    var L = {}, k = {};
    var i, y;
    ...
    var B = d.append("g").attr("class", "links"), f = d.append("g").attr("class", "episodes"), E = d.append("g").attr("class", "nodes");
    var Q = d3.select("#graph-info");
    d3.json(URL+"/api/projects/"+project_id+"/visual", function(X, Y) {
        T = d3.map(Y);
        q = d3.merge(T.values());
        x = {};
        A = d3.map();
    ...
    function O() {
        ...
        var ad = 180 + J, Z = 360 - J, ac = (Z - ad) / (T.get("themes").length-1);
    ...
    function G(Y, X) {
        ...
        if(L.node.type == "theme" || L.node.type == "perspective")
        {
            window.location.href = "" + L.node.slug;
            return;
        }
    }
    function n() {
    ...
    function C(Z) {
        ...
        if (ab && ab.type === "episode") {
            ...
            aa.append("text").attr("fill", N).attr("text-anchor", "middle").attr("y", (o + t)*-1).text(function(ae) {
                return "" //"CAPABILITY ##" + ae.episode
            })
        } else {
            if (ab && (ab.type === "theme" || ab.type === "perspective")) {
        ...
        X.enter().append("text").attr("text-anchor", "start").attr("x", a/-2 + t).attr("y", c / 2 - t).text("All Capabilities").attr("class", "all-episodes").on("click", O);
        X.exit().remove()
    }
    function D(Y) {
