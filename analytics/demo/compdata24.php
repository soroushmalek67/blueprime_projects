...
else
{
  ...
  $level = $row['Level'];
  $result = mysqli_query($con,"select floor(count(*)/2) half from ProjectCompanies Where ProjectID=$ProjectID");
  $row = mysqli_fetch_array($result);
  $half = $row['half'];
  //$half = 25;
  echo '{"episodes":[';
  $result = mysqli_query($con,"select distinct n.Code,n.Name from ProjectCompanies pc INNER JOIN NAICS n on SUBSTRING(pc.NAICSCode,1,$level) = n.Code Where ProjectID=$ProjectID");
  if (!$result)
  {
    echo "<script>window.location = 'index.html'</script>";
    exit();
  ...
  {
    $i = $i + 1;
    echo '{"type":"episode","name":"' . $row["Name"] . '","description":"' . $row["Name"] . '","episode":' . $i .',"date":"2012-05-05 23:50:11","slug":""';
    echo ',"links":[';
    $result1 = mysqli_query($con,"select IF(length(pc.Name)>25,concat(SUBSTRING(pc.Name,1,25),'...'),pc.Name) Name from ProjectCompanies pc Where ProjectID=$ProjectID AND SUBSTRING(pc.NAICSCode,1,$level)='". $row["Code"] . "'");
    if (!$result1)
    {
      echo "<script>window.location = 'index.html'</script>";
      exit();
...
//  $result = mysqli_query($con,"select pc.ID,pc.Name,concat(REPLACE(SUBSTRING(Description,1,100),'\n',', '),'...') Title from ProjectCompanies pc Where ProjectID=$ProjectID order by Name");
  $result = mysqli_query($con,"select pc.ID,IF(length(pc.Name)>25,concat(SUBSTRING(pc.Name,1,25),'...'),pc.Name) Name20, pc.Name,pc.Website,pc.FacebookURL,pc.TwitterURL,pc.LinkedInURL,OtherURL,REPLACE(REPLACE(SUBSTRING(Description,1,400),'www','<br/>'),'\"','') Title from ProjectCompanies pc Where ProjectID=$ProjectID order by Name");
  if (!$result)
  {
    echo "<script>window.location = 'index.html'</script>";
    exit();
 ...
 //     echo '{"type":"theme","name":"' . $row["Name"] . '","description":"<a href=index.htm>Facebook </a> ' . $row["Title"] . '","slug":"comp.php?ID=' . $row["ID"] . '"';
      echo '{"type":"theme","name":"' . $row["Name20"] . '","description":"' . $social . '<br/> ' . $title . '","slug":"comp24.php?ID=' . $row["ID"] . '"';
    else
      echo '{"type":"perspective","name":"' . $row["Name20"] . '","description":"' . $social . '<br/> ' . $title . '","slug":"comp24.php?ID=' . $row["ID"] . '","count":"10","group":"1"';
//      echo '{"type":"perspective","name":"' . $row["Name"] . '","description":"<a target="_blank" href="' . $row["FacebookURL"] . '"><img width="40px" src="img/facebook32.png" /></a> ' . $row["Title"] . '","slug":"comp.php?ID=' . $row["ID"] . '","count":"10","group":"1"';
    if ($i<mysqli_num_rows($result) && $i != $half-1)
      echo "},";
    else
      echo "}";
  }
