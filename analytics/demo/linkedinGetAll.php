    ...
    <?php
          ...
          if($_SESSION['oauth']['linkedin']['authorized'] === TRUE) {
            ...
            $OBJ_linkedin->setResponseFormat(LINKEDIN::_RESPONSE_JSON);
                ...
                {
                  $result = mysqli_query($con,"SELECT * FROM Companies Where ID >291");
                  while($row = mysqli_fetch_array($result))
                  {
                    $query    = '?sort=relevance&keywords=' . str_replace(" ","+",$row['Name']);
                    ...
                    if($response['success'] === TRUE) {
                      ...
                      for ($i = 0; $i < count($comps); $i++) {
                            include 'dbConn2.php';
                            $sql = "INSERT INTO CompanyLinkedIns (CompanyID,LinkedInID) VALUES (".$row['ID'].",".$comps[$i]['id'].")";
