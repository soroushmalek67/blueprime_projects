(function() {
    var a = 1000, c = 800, h = c, U = 250, K = 22, S = 20, s = 8, R = 110, J = 30, o = 15, t = 10, w = 1000, F = "elastic", N = "#0da4d3";
    var T, q, x, j, H, A, P;
    var L = {}, k = {};
    var i, y;
    ...
    d3.json("compdata.php", function(X, Y) {
        T = d3.map(Y);
        q = d3.merge(T.values());
        x = {};
        A = d3.map();
...
//        Q.html('<a href="/the-concept-map/">What\'s this????</a>');
    ...
    function C(Z) {
        ...
        X.enter().append("text").attr("text-anchor", "start").attr("x", a/-2 + t).attr("y", c / 2 - t).text("").attr("class", "all-episodes").on("click", O);
        X.exit().remove()
    }
    function D(Y) {
